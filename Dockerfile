...
WORKDIR /src
COPY ["Quiz Task.csproj", "."]
RUN dotnet restore "./Quiz Task.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./Quiz Task.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Quiz Task.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "Quiz Task.dll"]